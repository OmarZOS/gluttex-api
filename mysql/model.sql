...
CREATE TABLE IF NOT EXISTS `gluttex`.`provider_details` (
  ...
  `provider_details_name` VARCHAR(255) NULL,
...
CREATE TABLE IF NOT EXISTS `gluttex`.`person` (
  ...
  `person_blood_type_id` ENUM('A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-') NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `gluttex`.`conversation` (
  ...
  INDEX `fk_message_convo_idx` (`conversation_sender_user_id` ASC) VISIBLE,
