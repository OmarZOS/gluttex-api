...
class TestDocumentService:
    ...
    @pytest.fixture
    def document_service(self, mock_cart_service):
    ...
    def test_get_cart_by_cart_id(self, document_service, mock_cart_service, sample_cart, sample_cart_json):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen:
            mock_invoice_gen.cart_to_json.return_value = sample_cart_json
    ...
    def test_generate_invoice_html_success(self, document_service, mock_cart_service, sample_cart, sample_cart_json):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen, \
            ...
            mock_invoice_gen.cart_to_json.return_value = sample_cart_json
            mock_invoice_gen.from_json.return_value = Mock()
            mock_renderer.return_value.render_compact_invoice.return_value = "<html>Invoice HTML</html>"
    ...
    def test_generate_receipt_html_success(self, document_service, mock_cart_service, sample_cart, sample_cart_json):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen, \
            ...
            mock_invoice_gen.cart_to_json.return_value = sample_cart_json
            mock_invoice_gen.from_json.return_value = Mock()
            mock_renderer.return_value.render_compact_receipt.return_value = "<html>Receipt HTML</html>"
    ...
    def test_generate_invoice_pdf_success(self, document_service, mock_cart_service, sample_cart, sample_cart_json):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen, \
            ...
            mock_invoice_gen.cart_to_json.return_value = sample_cart_json
            mock_invoice_gen.from_json.return_value = Mock()
            mock_renderer.return_value.render_pdf_invoice.return_value = b"%PDF-1.4 Invoice PDF Content"
    ...
    def test_generate_receipt_pdf_success(self, document_service, mock_cart_service, sample_cart, sample_cart_json):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen, \
            ...
            mock_invoice_gen.cart_to_json.return_value = sample_cart_json
            mock_invoice_gen.from_json.return_value = Mock()
            mock_renderer.return_value.render_pdf_receipt.return_value = b"%PDF-1.4 Receipt PDF Content"
...
class TestDocumentServiceWithDataInput:
    ...
    def test_generate_invoice_from_data_html(self, mock_renderer, document_service, sample_invoice_data):
        ...
        mock_renderer.return_value.render_compact_invoice.return_value = "<html>Invoice from Data</html>"
        
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen:
            mock_invoice = Mock()
            mock_invoice_gen.from_json.return_value = mock_invoice
    ...
    def test_generate_receipt_from_data_html(self, mock_renderer, document_service, sample_receipt_data):
        ...
        mock_renderer.return_value.render_compact_receipt.return_value = "<html>Receipt from Data</html>"
    ...
    def test_generate_invoice_from_data_pdf(self, mock_renderer, document_service, sample_invoice_data):
        ...
        mock_renderer.return_value.render_pdf_invoice.return_value = b"%PDF-1.4 Invoice from Data"
    ...
    def test_generate_receipt_from_data_pdf(self, mock_renderer, document_service, sample_receipt_data):
        ...
        mock_renderer.return_value.render_pdf_receipt.return_value = b"%PDF-1.4 Receipt from Data"
...
class TestDocumentServiceWithQRCode:
    ...
    def test_generate_invoice_with_qr_code_html(self, mock_qr_code, mock_renderer, document_service, sample_invoice_data):
        ...
        mock_renderer.return_value.render_compact_invoice.return_value = "<html>Invoice with QR Code</html>"
    ...
    def test_generate_receipt_with_qr_code_html(self, mock_qr_code, mock_renderer, document_service, sample_receipt_data):
        ...
        mock_renderer.return_value.render_compact_receipt.return_value = "<html>Receipt with QR Code</html>"
    ...
    def test_generate_invoice_with_qr_code_pdf(self, mock_qr_code, mock_renderer, document_service, sample_invoice_data):
        ...
        mock_renderer.return_value.render_pdf_invoice.return_value = b"%PDF-1.4 Invoice with QR Code"
...
class TestDocumentServiceErrorHandling:
    ...
    def test_invoice_generation_invalid_cart_data(self, document_service, mock_cart_service, sample_cart):
        ...
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen:
            mock_invoice_gen.cart_to_json.side_effect = Exception("Invalid cart data")
    ...
    def test_document_generation_empty_items(self, document_service, mock_cart_service, sample_cart):
        """Test document generation with empty items list"""
        sample_cart.ordered_item = []
        mock_cart_service.get_cart_by_id.return_value = sample_cart
        
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen:
            mock_invoice_gen.cart_to_json.return_value = {
                "cart_id": 123,
                "items": []
            }
            mock_invoice_gen.from_json.return_value = Mock()
            
            with patch('services.document_service.get_renderer') as mock_renderer:
                mock_renderer.return_value.render_compact_invoice.return_value = "<html>Empty Invoice</html>"
...
# Integration test with actual database fixtures
@pytest.mark.integration
class TestDocumentServiceIntegration:
    """Integration tests for DocumentService using actual database"""
    
    @pytest.fixture
    def document_service(self):
        """Create real document service"""
        return DocumentService()
    
    def test_get_cart_from_db(self, document_service, populated_db):
        """Test getting cart from database"""
        cart = populated_db['cart']
        
        with patch('services.document_service.InvoiceGenerator') as mock_invoice_gen:
            mock_invoice_gen.cart_to_json.return_value = {"cart_id": cart.cart_id}
            
            result = document_service.get_cart_for_document(cart_id=cart.cart_id)
