...
async def create_user(user_data):
    ...
    response = await send_post_request(url, user_data)

    if response.status_code != 200:
        raise Exception(response.content)
    return  json.loads(response.content)

async def login_for_access_token( app_user : AppUser_API):
    form_data = {
    ...
    return await send_post_request(url, form_data, headers)
