...
from features.recipe.recipe_fetch import fetch_all_recipe, fetch_recipe_by_id, get_ingredients, get_recipe_categories, get_recipe_image_by_id, get_recipes_by_category_id
...
def get_recipes_by_category(category_id: int):
    ...
    return res@recipe_router.get("/recipe/category/{category_id}")
def get_recipes_by_category(category_id: int):
    try:
        res = get_recipes_by_category_id(category_id)
    except Exception as e:
...
def get_Ingredients():
    try:
        ...
        res = JSONResponse(
        status_code=status.HTTP_406_NOT_ACCEPTABLE,
        content=jsonable_encoder({"detail": str(e), "Error": "Couldn't fetch ingredients."}),
    )
    return res
